...
// THose were added by Yi Shen and should NOT be used. 
// Won't work because line: 892 util.c
/*`
  ...
  if((idx = get_str_index(table, size, "P")) >= 0) {
      if(sscanf(table[idx].value, "%s", config->p_step_outfile) != 1)
        fatal("invalid format for configuration  parameter p_step_outfile\n");
  } else {
      strcpy(config->p_step_outfile, NULLFILE);
  }
  if((idx = get_str_index(table, size, "V")) >= 0) {
      if(sscanf(table[idx].value, "%s", config->v_step_outfile) != 1)
        fatal("invalid format for configuration  parameter v_step_outfile\n");
  } else {
      strcpy(config->v_step_outfile, NULLFILE);
  }
  if((idx = get_str_index(table, size, "B")) >= 0) {
      if(sscanf(table[idx].value, "%s", config->b_outfile) != 1)
        fatal("invalid format for configuration  parameter b_outfile\n");
  } else {
      strcpy(config->b_outfile, NULLFILE);
  }
  if((idx = get_str_index(table, size, "I")) >= 0) {
      if(sscanf(table[idx].value, "%s", config->i_step_outfile) != 1)
        fatal("invalid format for configuration  parameter i_step_outfile\n");
  } else {
      strcpy(config->i_step_outfile, NULLFILE);
  }
  */
  if((idx = get_str_index(table, size, "v")) >= 0) {
