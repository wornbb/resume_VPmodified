...
void PDN_trans_analyze(model_t *model, status_t *status)
{
  ...
  dump_curr_grid(model);
  dump_volt_grid(model);
  for(i=0; i<m; i++)
...
// added by Yi Shen 2019-02-12
void dump_b_matrix(model_t *model){};
void dump_p_step_grid(model_t *model){};
void dump_curr_grid(model_t *model)
{
  int i, j;
  double temp, vnoise;
  double max = -LARGENUM;
  ...
  int nbr = 2*nr*nc - nr - nc;
  //print grid voltage trace
  if (strcmp(model->config.i_step_outfile, NULLFILE)){
	fp_v = fopen(model->config.i_step_outfile, "a");
      if (!fp_v) {
          sprintf (str,"error: %s could not be opened for writing\n", model->config.i_step_outfile);
          fatal(str);
      }
      for(i = 0; i < nr; i++)
        for(j = 0; j < nc; j++){
            cur = i*nc + j; 
            fprintf(fp_v, "%.7f ", model->last_trans[nvp+cur+nc*nr]);
        }
      fprintf(fp_v, "\n");
      fclose(fp_v);
  }
}
void dump_volt_grid(model_t *model)
{
  int i, j;
  double temp, vnoise;
  double max = -LARGENUM;
  ...
  int nbr = 2*nr*nc - nr - nc;
  //print grid voltage trace
  if (strcmp(model->config.v_step_outfile, NULLFILE)){
      fp_v = fopen (model->config.v_step_outfile, "a");
      if (!fp_v) {
          sprintf (str,"error: %s could not be opened for writing\n", model->config.v_step_outfile);
          fatal(str);
      }
      for(i = 0; i < nr; i++)
        for(j = 0; j < nc; j++){
            cur = i*nc + j; 
            fprintf(fp_v, "%.7f ", model->last_trans[nvp+cur]);
        }
      fprintf(fp_v, "\n");
      fclose(fp_v);
  }
}
void dump_power_grid(model_t *model, model_vector_t *g, char *file)
{
  int i, j;
  char str[STR_SIZE];
  FILE *fp;
  strcpy(str, model->config.gridvol_file);
  strcat(str, "powerG");
  fp = fopen(str, "a");
  int nr = model->rows;
  int nc = model->cols;
  int len = nr * nc;
// We don't have more than 1 layer since we are not studying 3D chips
  int l = 0;
  // g is the power grid, which is accessible as a matrix
  // it is defined in "PDN_sim.c" with "dcuboid_PDN"
      for(i=0; i<model->rows; i++)
        for(j=0; j<model->cols; j++) {
    	    fprintf(fp, "%f\t", g->cuboid[l][i][j]);
        }
  fprintf(fp, "\n");
  fclose(fp);
}
